...
public class CommandSearchShows implements Command {
    ...
    public void execute(IMessage message, String content, String[] args, IUser author, IGuild guild, List<Long> roleIDs, boolean isPrivate, IDiscordClient client, TestBot bot) {
        ...
        if (showData != null) {
            ...
            initialMessage.edit("Found show!", embed.withColor(Util.getBotColor()).build());
        } else {
            initialMessage.edit("Oops", new EmbedBuilder().withDesc("Error: Show not found").withColor(Util.getBotColor()).build());
            Util.simpleEmbed(client.getChannelByID(TestBot.ERRORLOG_CH_ID), "Couldn't find show " + showName + " in " + guild.getName() + "/" + message.getChannel().getName());
        }
    }
